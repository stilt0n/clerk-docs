...
Starts a sign-in flow that uses the Metamask browser extension to authenticate the user using their public web3 wallet address.
...
Starts a sign-in flow that uses the Coinbase Wallet browser extension to authenticate the user using their public web3 wallet address.

```typescript
function authenticateWithCoinbase(): Promise<SignInResource>
```
...
Starts a sign-in flow that authenticates the user against their public web3 wallet address.
